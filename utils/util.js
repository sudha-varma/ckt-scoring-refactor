 ...
 * mongoose will understand
 * ["createdAt", "-updatedAt"] -> {"createdAt": 1, "updatedAt": -1}
 * @static
 * @method parseSortBy
 * @param sortBy
 * @return {Object}
 */
Util.parseSortBy = sortBy => {
  let sort = {};
  if (sortBy) {
    // Parse sort keys
    sortBy.forEach(key => {
...
/**
 * Adding or updating reference based on feedSource
 * @static
 * @method addOrUpdateReference
 * @param existingReferences Existing reference array
 * @param newReference New Reference
 * @return {undefined}
 */
Util.addOrUpdateReference = (existingReferences, newReference) => {
  // Checking if reference exist with the feedSource
  const existingObj = existingReferences.find(
    obj => obj && obj.feedSource === newReference.feedSource,
  );
  if (!existingObj) {
    // Adding new reference if not exist
    existingReferences.push(newReference);
  } else {
    // Updating reference key otherwise
    existingObj.key = newReference.key;
  }
};
