...
async function find({ query, autoFormat = true }) {
  const res = await commonService.find({ Model: Squad, query, autoFormat });
  return res;
}
...
async function create({ data, autoFormat = true }) {
  const res = await commonService.create({ Model: Squad, data, autoFormat });
  return res;
}
...
async function removeById({ id, autoFormat = true }) {
  const res = await commonService.removeById({ Model: Squad, id, autoFormat });
  return res;
}
