...
async function find({ query, autoFormat = true }) {
  const res = await commonService.find({ Model: ScoreCard, query, autoFormat });
  return res;
}
...
/**
 * Pseudo delete document
 * @property {string} matchId - matchId for which scorecard to be removed.
 * @property {boolean} autoFormat - false if formatted output not needed.
 * @returns {document}
 */
 ...
 * @property {string} matchId - matchId for which scorecard to be fetched.
 * @property {string} errKey - key for which error object will be generated.
 * @property {boolean} autoFormat - false if formatted output not needed.
 * @returns {document}
 */
async function findByMatchId({ matchId, errKey, autoFormat = true }) {
  ...
  const existingDoc = await ScoreCard.findOne({
    matchId,
    status: "active",
  });
