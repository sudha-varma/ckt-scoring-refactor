...
async function find({ query, autoFormat = true }) {
  const res = await commonService.find({ Model: Series, query, autoFormat });
  return res;
}
...
async function create({ data, autoFormat = true }) {
  const res = await commonService.create({ Model: Series, data, autoFormat });
  return res;
}
...
async function removeById({ id, autoFormat = true }) {
  const res = await commonService.removeById({ Model: Series, id, autoFormat });
  return res;
}
